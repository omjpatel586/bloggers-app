...
export const View = () => {
    ...
    useEffect(() => {
        axios.post("https://omcdmiweb.000webhostapp.com/React.js_Project_API./getBlogs.php", fdata)
            .then(function (fetch) {
                setdata([...fetch.data])
            })
    ...
    const handleBtn = () => {
        ...
        data.map((i, j) => {
            if (document.getElementById("customSwitches" + j + "a").checked === true) {
                ...
                fdata.append("uid", window.localStorage.getItem("uid"))
                axios.post("https://omcdmiweb.000webhostapp.com/React.js_Project_API./editBlogs.php", fdata)
                    .then(function () {
                                                ...
                                                </tr>
                                                {
                                                    data.map((i, j) =>
                                                        <tr className="position-relative">
                                                            <td>{i.id}</td>
                                                            <td>
                                                                <h1 className="fw-bolder fs-6" id={"editth1" + j}>{i.title}</h1>
                                                                <button className="hebtn btn text-danger position-absolute bottom-0" onClick={() => { handleEdit(i.id) }}>Edit</button>
                                                                <button className="hcbtn btn text-danger position-absolute bottom-0" style={{left: "100px"}} onClick={()=>{ handleConfirm(i.id) }}>Confirm</button>
                                                                <textarea rows={5} cols={15}
                                                                    defaultValue={i.title} style={{ display: "none" }} id={"editt" + j}></textarea>
                                                            </td>
                                                            <td>
                                                                <p id={"editpd" + j}>
                                                                ...
                                                                <textarea rows={5} cols={10}
                                                                    defaultValue={i.description} style={{ display: "none" }} id={"editd" + j}></textarea>
                                                            </td>
                                                            <td>
                                                            ...
                                                            <td>
                                                                <div className="image1">
                                                                    ...
                                                                    <input type="file"
                                                                        style={{ display: "none" }} id={"editf" + j}></input>
                                                                </div>
                                                            </td>
